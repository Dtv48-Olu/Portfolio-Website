...
function repoToProject(repo) {
  ...
  if (repo.topics) techStack.push(...repo.topics.filter((t) => t && t.length <= 20));
...
function mergeProjects(manualProjects, autoProjects) {
  const map = new Map();
  manualProjects.forEach((project) => map.set(project.id, project));
      ...
      autoProjects = repos
        .filter(
          (repo) =>
            ...
            !repo.disabled &&
            !repo.fork &&
            (repo.topics?.some((topic) => TOPICS_INCLUDE.includes(topic)) ||
